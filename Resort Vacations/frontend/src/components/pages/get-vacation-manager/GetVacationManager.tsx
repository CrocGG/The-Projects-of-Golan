...
export default function GetVacationManager() {

    useTitle('Vacations - Manager');
    ...
    }, [wholeVacations, vacationService])


    async function deleteClick(event: React.MouseEvent<HTMLButtonElement>) {
        try {
            if (confirm('Are you sure?')) {
                await vacationService.annihilateVacation(event.currentTarget.value)
                alert('Annihilated!!')
            }
    ...
    return (
        <div className='vacation-list'>
            <h1>Vacation List</h1>
            <div className="vacations-container-manager">
                {wholeVacations.map(({ id, destination, description, beginningDate, endingDate, price, imageUrl }) => (
                    <div key={id} className="single-container-manager">
                        ...
                        <NavLink to={`/vacation/vacation-edit/${id}`}>
                            <button>Edit</button>
                        </NavLink>
                        </div>
